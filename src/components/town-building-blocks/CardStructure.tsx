...
const CardStructure: React.FC<{ card: Card; genericMaidUrl?: string }> = ({
	card,
	genericMaidUrl,
}) => {
	return (
					...
					<div className={`card-background ${card.set}`}>
						<div className="single-card-grid">
							...
							<div className={`${card.set} image`}>
								<img src={`${genericMaidUrl}`} />
							</div>
							<div className="bottomSection">
								{card.cardDraw > 0 && (
									<div className={`card-draw ${card.set}`}>
										<div className={`hasValue ${card.set}`}>
											+{card.cardDraw}
										</div>
										<div className={`iconEnabled ${card.set}`}>
											<svg
												viewBox="0 0 24 24"
								...
								{card.cardDraw === 0 && (
									<div className={`card-draw ${card.set}`}>
										...
										<div className={`iconDisabled ${card.set}`}>
											<svg
												viewBox="0 0 24 24"
												xmlns="http://www.w3.org/2000/svg"
											>
								...
								{card.servings > 0 && (
									<div className={`servings ${card.set}`}>
										<div className={`${card.set} iconEnabled`}>
											<svg
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 0 512 512"
											>
										...
										<div className={`${card.set} hasValue`}>
											+{card.servings}
										</div>
									</div>
								)}
								...
								{card.love > 0 && (
									<div className={`${card.set} love`}>
										<div className={`${card.set} hasValue`}>+{card.love}</div>
										<div className={`${card.set} iconEnabled`}>
											<svg
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 0 512 512"
								...
								{card.love === 0 && (
									<div className={`love ${card.set}`}>
										<div className={`hasNoValue ${card.set}`}>+0</div>
										<div className={`${card.set} iconDisabled`}>
											<svg
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 0 512 512"
											>
								...
								{card.employs === 0 && (
									<div className={`${card.set} employs`}>
										...
										<div className={`${card.set} hasNoValue`}>+0</div>
									</div>
								)}
							</div>
						</div>
